...
from agents.GaussianIQLagent import GaussianActor
from utils.minari_replaybuffer import load_minari_dataset

ckpt_path = f"checkpoints/iql_symmetric_best.pt"
...
@torch.no_grad()
def act(actor, obs, replay, deterministic: bool = True) -> np.ndarray:
    obs_t = torch.as_tensor(obs, device=device, dtype=torch.float32).unsqueeze(0)
    obs_n = replay.normalize_obs(obs_t)
    act = actor.act(obs_n, deterministic=False)
    return act.squeeze(0).cpu().numpy()

action = act(actor, obs, replay)
...
while not done:
    for i in range(n_samples):
        new_action = act(actor, obs, replay) #[,act_dim]
        data[i] = new_action
