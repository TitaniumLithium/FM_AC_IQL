...
class GaussianActor(nn.Module):
    def __init__(self, obs_dim: int, act_dim: int, hidden_dims=(256, 256), log_std_min=-5.0, log_std_max=2.0,chunk_len=4):
        super().__init__()
        self.net = mlp(obs_dim, hidden_dims, 2 * act_dim * chunk_len)
    ...
    def forward(self, obs: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        obs [B, obs_dim]
        '''
        out = self.net(obs) # [B, 2 *act_dim*chunk_len]
        out = out.reshape(-1, self.chunk_len, 2 * self.act_dim) # [B, chunk_len, 2 * act_dim]
        mean, log_std = torch.chunk(out, 2, dim=-1) # [B, chunk_len, act_dim]
    ...
    def dist(self, obs: torch.Tensor) -> torch.distributions.Distribution:
        ...
        base = torch.distributions.Independent(
            torch.distributions.Normal(mean, log_std.exp()), 2
        )
        # 2: Cal chunk&act_dim as event 
        # [B chunk_len act_dim] ->logprob -> [B]
        # this treat action chunk as a whole process
    ...
    def sample(self, obs: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        dist = self.dist(obs) # [B chunk_len*act_dim]
        action_chunk = dist.rsample() # [B, chunk_len, act_dim]
        log_prob = dist.log_prob(action_chunk) #[B]
        return action_chunk, log_prob

    def log_prob(self, obs: torch.Tensor, action_chunk: torch.Tensor) -> torch.Tensor:
        '''
        [B chunk_len act_dim] ->logprob -> [B]
        '''
        action_chunk = torch.clamp(action_chunk, -0.999999, 0.999999)
        dist = self.dist(obs)
        return dist.log_prob(action_chunk)

    def act(self, obs: torch.Tensor, deterministic: bool = True) -> torch.Tensor:
        '''
        obs [B, obs_dim]
        out [B, chunk_len, act_dim]
        '''
...
class IQLAgent:
    def __init__(
        ...
        self.actor = GaussianActor(obs_dim, act_dim,chunk_len=chunk_len).to(device)
        self.critic = Critic(obs_dim, act_dim, chunk_len=chunk_len).to(device)
    ...
    def actor_loss(self, obs: torch.Tensor, actions: torch.Tensor) -> Tuple[torch.Tensor, Dict[str, float]]:
        ...
        log_prob = self.actor.log_prob(obs, actions) # [B]
